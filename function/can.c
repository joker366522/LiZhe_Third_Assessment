...
void CAN2_Mode_Init(void)
{
	...
	/*GPIO的配置*/
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12|GPIO_Pin_13; //RX、TX
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	...
	/*引脚复用*/
	GPIO_PinAFConfig(GPIOB,GPIO_PinSource12,GPIO_AF_CAN2); //复用
	...
	/*配置过滤器*/
	CAN_FilterInitStructure.CAN_FilterNumber = 14; //指定过滤器为0
	CAN_FilterInitStructure.CAN_FilterMode = CAN_FilterMode_IdMask;//标识符列表模式
	CAN_FilterInitStructure.CAN_FilterScale = CAN_FilterScale_32bit;//过滤器位宽为32
	CAN_FilterInitStructure.CAN_FilterIdHigh      = 0x0000; //要过滤的ID高位和低位
	CAN_FilterInitStructure.CAN_FilterIdLow       = 0x0000;
	...
	/*配置中断*/
	CAN_ITConfig(CAN2,CAN_IT_FMP0,ENABLE);//FIF00信息挂号允许，使能
	NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);
...
void CAN1_Send_Msg(int a1,int a2,int a3,int a4)
{
	CanTxMsg txMessage;
...
CanTxMsg txMessage;
void CAN2_Send_Msg(int a,int b)
{
	lalala = 1;
	...
	txMessage.RTR = CAN_RTR_Data; //数据帧而不是遥控帧
	txMessage.IDE = CAN_Id_Standard; //使用标准标识符
	txMessage.StdId = 0x1FF; //标准标识符
	txMessage.DLC = 0x08; //消息的数据长度为8个字节
	
	txMessage.Data[0] = (int)(a>>8);
    txMessage.Data[1] = a; //旋转电机
    txMessage.Data[2] = (int)(b>>8);
    txMessage.Data[3] = b; //点头电机
